...
BASE_URL="https://raw.githubusercontent.com/AlexStrew/MTPROTO_FIX_By_MEKO/main"
    ...
    case "$1" in
        ...
        -fix-port)
            if [[ "$2" =~ ^[0-9]+$ ]] && [ "$2" -ge 1 ] && [ "$2" -le 65535 ]; then
                FIX_PORT="$2"
                shift 2
            elif [ "$(echo "$2" | tr '[:upper:]' '[:lower:]')" = "all" ]; then
                FIX_PORT="all"
                shift 2
            else
                echo -e "${RED}[✗]${NC} Неверный порт: $2 (допустимы: 1-65535 или all)"; exit 1
            fi
            ;;
        -port)
            if [[ "$2" =~ ^[0-9]+$ ]] && [ "$2" -ge 1 ] && [ "$2" -le 65535 ]; then
        ...
        -h|--help)
            ...
            echo -e "                          Можно указать 'all' для установки на все порты (включая SSH)"
            echo -e "    -port <порт>           порт для прокси (и для фикса, если не задан -fix-port)"
            echo -e "                          Можно указать 'all' для работы на всех портах"
            ...
            echo ""
            echo -e "    # Фикс на ВСЕ порты включая SSH (обход фриз-бана ТСПУ)"
            echo -e "    curl ... | sudo bash -s -- -fix -fix-port all"
            echo ""
...
if [[ -n "$FLAG_TELEMT" || -n "$FLAG_ZIG" || -n "$FLAG_MTG" || -n "$FLAG_FIX" || -n "$FLAG_WEB" ]]; then
    ...
    if [[ -n "$FLAG_FIX" && -z "$FLAG_NO_FIX" ]]; then
        ...
        if [ -z "$FIX_TYPE" ]; then
            # В авто-режиме (переданы флаги) — дефолт v3 без вопросов
            FIX_TYPE="v3"
            log_info "Тип фикса не указан, используем v3 (рекомендуется)"
        fi
    fi
